  ...
  const toggleFavorite = (id) => {
    setFavorites((prevFavorites) => 
      prevFavorites.includes(id)
        ? prevFavorites.filter((favId) => favId !== id) //cancel favorite
        : [...prevFavorites, id] //add favorite
        );
      };
    ...
    {cards.map((card) => (
      ...
      >
        ...
        <div className="relative">
          ...
          />
          {/* Heart Icon */}
          <div className="absolute top-4 right-4 bg-white bg-opacity-70 p-2 rounded-full cursor-pointer"
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
              strokeWidth={2}
              stroke={favorites.includes(card.id) ? "red" : "black"}
              className="w-6 h-6"
            >
              <path
                strokeLinecap="round"
